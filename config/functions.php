    ...
    try {
        ...
        if (!isset($settings)) {
            ...
            foreach ($query as $row) {
                $settings[$row->name] = (is_serialized($row->value))
...
{
    $protected = ['empty', 'ms_license_response_result'];
    $iterator  = new RecursiveIteratorIterator(
        new RecursiveDirectoryIterator(CACHE, FilesystemIterator::SKIP_DOTS),
        RecursiveIteratorIterator::CHILD_FIRST
    );
    foreach ($iterator as $fileInfo) {
        if ($fileInfo->isFile() && !in_array($fileInfo->getFilename(), $protected, true)) {
            unlink($fileInfo->getRealPath());
        }
    }
}

function emptyLogs(): void
{
    $protected = ['empty'];
    $iterator  = new RecursiveIteratorIterator(
        new RecursiveDirectoryIterator(LOGS, FilesystemIterator::SKIP_DOTS),
        RecursiveIteratorIterator::CHILD_FIRST
    );
    foreach ($iterator as $fileInfo) {
        if ($fileInfo->isFile() && !in_array($fileInfo->getFilename(), $protected, true)) {
...
function generate_random_string(int $length = 10, bool $special = false): string
{
    $specialChars = '~!@#$%^&*(){}[],./?';
    $alphaNum     = 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';

    $pool = $special ? $alphaNum . $specialChars : $alphaNum;
    $max  = strlen($pool) - 1;

    $string = '';
    for ($i = 0; $i < $length; $i++) {
        $string .= $pool[random_int(0, $max)]; // cryptographically secure CSPRNG
    }
...
/**
 * Check whether an IP address belongs to Cloudflare's published IPv4 ranges.
 * Ranges sourced from: https://www.cloudflare.com/ips-v4
 * Update these periodically or fetch dynamically from the URL above.
 *
 * @param string $ip
 * @return bool
 */
function is_cloudflare_ip(string $ip): bool
{
    ...
    return \App\Library\IpUtils::checkIp($ip, $cfRanges);
}

/**
 * Get the real client IP address.
 *
 * Only trusts proxy-injected headers (CF-Connecting-IP, Fastly) when the
 * *actual* TCP connection (REMOTE_ADDR) comes from a known proxy IP range.
 * This prevents IP spoofing via forged request headers.
 *
...
function get_ip(): string
{
    ...
    // Only trust Cloudflare's header when the connection actually came from Cloudflare
    if (!empty($_SERVER['HTTP_CF_CONNECTING_IP']) && is_cloudflare_ip($remoteAddr)) {
        $candidate = trim(explode(',', $_SERVER['HTTP_CF_CONNECTING_IP'])[0]);
    // Only trust Fastly header when the connection came from a Fastly IP
    } elseif (!empty($_SERVER['HTTP_FASTLY_CLIENT_IP'])) {
        // Fastly uses REMOTE_ADDR as its egress — trust only if non-public
        $candidate = $remoteAddr;
    } else {
        // For all other cases use the direct connection IP
        $candidate = $remoteAddr;
    }

    // Final validation — ensure it's actually a valid IP
    $ip = filter_var(trim($candidate), FILTER_VALIDATE_IP) ?: '0.0.0.0';
