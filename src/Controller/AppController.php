...
class AppController extends Controller
{
    ...
    protected function _AuthenticateCookieUser()
    {
        ...
        $rememberMeCookie = $this->getRequest()->getCookie('RememberMe');
        if (!$rememberMeCookie || strpos($rememberMeCookie, ':') === false) {
            return null;
        }

        [$selector, $authenticator] = explode(':', $rememberMeCookie, 2);
        ...
        if (!$rememberToken) {
            setcookie('RememberMe', '', ['expires' => -1, 'path' => '/', 'secure' => true, 'httponly' => true, 'samesite' => 'Lax']);
